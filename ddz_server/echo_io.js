...
var sys = require("util"), fs = require("fs"), url = require("url"), path = require("path"), http = require("http"), ws = require('node-websocket-server'), querystring = require('querystring'), socket_io = require('socket.io'), eventproxy = require('eventproxy');
														...
														if (oldtnumb != tnumb) {
															...
															for ( var i = 0; i < tables[oldtnumb]['plays'].length; i++) {
																if (tables[oldtnumb]['plays'][i] != null) {
																	...
																	io.sockets.sockets[tables[oldtnumb]['plays'][i]['uid']]
																			.send(
																					//tables[oldtnumb]['plays'][i]['uid'],
																					'[{"uid":"'
																							+ uid
															...
															if (tables[tnumb]['plays'][i] != null) {
																...
																io.sockets.sockets[tables[tnumb]['plays'][i]['uid']]
																		.send(
																				...
																				+ '","gameState":3,"ctag":'
																				+ ctag
																				+ ',"ctype":23},{"total":15,"uid":0,"ctag":0,"ctype":13}]');
															}
														}
													} else {
														for ( var i = 0; i < tables[tnumb]['plays'].length; i++) {
